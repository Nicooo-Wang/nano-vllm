...
"""
    ...
    def __init__(
        ...
        assert num_embeddings % self.tp_size == 0, \
            f"词表大小({num_embeddings})必须能被GPU数({self.tp_size})整除"

        # 保存配置
        self.num_embeddings = num_embeddings  # 总词表大小
        self.num_embeddings_per_partition = num_embeddings // self.tp_size  # 每个GPU的词表大小

        # 计算当前GPU负责的词表范围
        self.vocab_start_idx = self.num_embeddings_per_partition * self.tp_rank  # 起始词ID
        self.vocab_end_idx = self.vocab_start_idx + self.num_embeddings_per_partition  # 结束词ID
...
class ParallelLMHead(VocabParallelEmbedding):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.tp_size > 1:
            ...
            # 主GPU拼接各GPU的部分logits
            if self.tp_rank == 0:
                logits = torch.cat(all_logits, -1)  # 在最后一个维度拼接
