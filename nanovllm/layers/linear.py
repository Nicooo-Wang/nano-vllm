...
def divide(numerator: int, denominator: int) -> int:
    ...
    assert numerator % denominator == 0, \
        f"{numerator}不能被{denominator}整除，张量并行要求均匀分布"
...
class ColumnParallelLinear(LinearBase):
    ...
    def __init__(
        ...
        super().__init__(
            input_size,
            divide(output_size, tp_size),  # 每个GPU的输出维度
            bias,
            tp_dim=0,  # 按列（输出维度）并行
        )
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def weight_loader(
        ...
        assert loaded_shard_id in ["q", "k", "v"], \
            f"loaded_shard_id必须是'q'、'k'或'v'，得到{loaded_shard_id}"
...
- 通信开销与输出大小成正比
    ...
    def __init__(
        ...
        # 输入维度按GPU数分片
        super().__init__(
            divide(input_size, tp_size),  # 每个GPU的输入维度
            output_size,
            bias,
            tp_dim=1,  # 按行（输入维度）并行
        )
