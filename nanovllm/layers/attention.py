...
def store_kvcache(
    ...
    assert key.stride(1) == head_dim and value.stride(1) == head_dim, \
        "K和V的第二维度步长必须等于head_dim"
    assert k_cache.stride(1) == D and v_cache.stride(1) == D, \
        "KV-cache的第二维度步长必须等于num_heads * head_dim"

    # 验证slot_mapping长度
    assert slot_mapping.numel() == N, \
        f"slot_mapping长度({slot_mapping.numel()})必须等于N({N})"

    # 调用Triton内核
    # [(N,)]表示启动N个线程块，每个线程块处理一个token
    store_kvcache_kernel[(N,)](
        key, key.stride(0),  # 当前K及其步长
        value, value.stride(0),  # 当前V及其步长
