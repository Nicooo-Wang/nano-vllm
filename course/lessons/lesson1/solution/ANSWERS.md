...
### 1.1 参考代码（逐字摘自 `solution/lab_solved.py:41-57`）
...
### 3.1 参考代码（逐字摘自 `solution/lab_solved.py:59-70`）
...
|---------------|------|--------------------------|
| `def traced_postprocess(...)`（Task 1，`lab.py:50-63`，def 在第 50 行） | TODO(student) — Task 1 (trace)：记录本步到 `_trace`，再调用原始 `postprocess` | `lab_solved.py:41-57` 整个函数体（见上 §1.1） |
| `def summarize_request(seq)`（Task 3，`lab.py:64-70`，def 在第 64 行） | TODO(student) — Task 3 (small wrapper)：返回 `(num_prompt_tokens, num_completion_tokens, total_steps)` | `lab_solved.py:59-70` 整个函数体（见上 §3.1） |
...
- [ ] §1.1 代码与 `lab_solved.py:41-57` **逐字一致**（含 docstring）。
- [ ] §3.1 代码与 `lab_solved.py:59-70` **逐字一致**（含 docstring）。
